...
Before deploying modules you will need to create Kafka server with:
`kubectl run kafka-release-client --restart='Never' --image docker.io/bitnami/kafka:2.8.1-debian-10-r0 --namespace default --command -- sleep infinity`

then access `kafka-release-client` with:
`kubectl exec --tty -i kafka-release-client --namespace default -- bash`

and create Producer and Consumer:

PRODUCER:
  `kafka-console-producer.sh --broker-list kafka-release-0.kafka-release-headless.default.svc.cluster.local:9092 --topic location`

CONSUMER:
  `kafka-console-consumer.sh --bootstrap-server kafka-release.default.svc.cluster.local:9092 --topic location --from-beginning`
...
5. `kubectl apply -f deployment/udaconnect-frontend.yaml` - Set up the service and deployment for the web app
6. `kubectl apply -f deployment/udaconnect-location-consumer-kafka.yaml` - Set up the service and deployment for Kafka consumer
7. `kubectl apply -f deployment/udaconnect-location-producer-grpc-kafka.yaml` - Set up the service and deployment for gRPC and Kafka producer
...
### Verifying it Works
Once the project is up and running, you should be able to see mentioned deployments and services in Kubernetes:
`kubectl get pods` and `kubectl get services` - should both return pods and services shown in docks images.


These pages should also load on your web browser:
* `http://localhost:30001/` - OpenAPI Documentation
* `http://localhost:30001/api/` - Base path for API
* `http://localhost:30003/` - Frontend ReactJS Application
